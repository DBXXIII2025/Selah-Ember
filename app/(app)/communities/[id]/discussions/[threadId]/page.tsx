...
type CommunityDiscussionThreadPageProps = {
...
export default async function CommunityDiscussionThreadPage({ params, searchParams }: CommunityDiscussionThreadPageProps) {
  const { id, threadId } = await params;
  const { message } = await searchParams;
  const [communityData, threadData] = await Promise.all([getCommunityThreads(id), getDiscussionThread(threadId)]);
  const thread = threadData.thread;

  if (communityData.state === "missing") {
    notFound();
  }
    ...
    return (
      <section className="px-6 py-12 sm:px-10 lg:px-16">
        <div className="mx-auto max-w-3xl rounded-2xl border border-[#ead6c5] bg-white/75 p-8 text-center shadow-sm">
          ...
          <Link href={`/communities/${id}/discussions`} className="mt-6 inline-flex text-sm font-semibold text-[#8a3f1e]">
            Back to discussions
          </Link>
        </div>
      </section>
  ...
  if (threadData.state === "non_member") {
    return (
      <section className="px-6 py-12 sm:px-10 lg:px-16">
        <div className="mx-auto max-w-3xl rounded-2xl border border-[#ead6c5] bg-white/75 p-8 text-center shadow-sm">
          <h1 className="text-3xl font-semibold">Join this community to view discussions</h1>
  ...
  if (!thread || thread.community_id !== id) {
    return (
      <section className="px-6 py-12 sm:px-10 lg:px-16">
        <div className="mx-auto max-w-3xl rounded-2xl border border-[#ead6c5] bg-white/75 p-8 text-center shadow-sm">
  ...
  const returnPath = `/communities/${id}/discussions/${threadId}`;
  ...
  return (
    <section className="px-6 py-12 sm:px-10 lg:px-16">
      <div className="mx-auto max-w-4xl">
        <Link href={`/communities/${id}/discussions`} className="text-sm font-semibold text-[#8a3f1e] hover:text-[#b94f22]">
          Back to discussions
        </Link>
        {message ? (
        ...
        <article className="mt-8 rounded-2xl border border-[#ead6c5] bg-white/75 p-8 shadow-sm">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[#b94f22]">Community thread</p>
